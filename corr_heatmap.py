...
# Oblicz macierz korelacji między wszystkimi parami
n = len(rankings)
...
for i in range(n):
    for j in range(n):
        if i == j:
            ...
            corr_matrix[i, j] = pm.correlations.spearman(rankings[i], rankings[j])

# LLM+Sugestie interpretujemy jako LLM+Sugestie(V1) z PDF 
